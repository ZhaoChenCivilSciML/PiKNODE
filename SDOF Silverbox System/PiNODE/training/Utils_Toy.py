...
def train(mdl_hysteresis, x, dx, ddxg, epochs, writer, source_seq, target_seq, dt, open_loop_horizon = None):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        loss_x_tr, loss_dx_tr, loss_hys_tr, z_t_1_seq_tr_pred, x_t_1_seq_tr_pred, dx_t_1_seq_tr_pred = comprehensive_forward_pass(mdl_hysteresis, x_t_seq_tr,
         x_t_1_seq_tr, dx_t_seq_tr, dx_t_1_seq_tr, ddxg_t_seq_tr, ddxg_t_1_seq_tr, target_seq, dt, x_mean, x_std,
            dx_mean, dx_std, ddxg_mean, ddxg_std, loss_fn)

        # total loss
        loss_tr = loss_x_tr + loss_dx_tr + loss_hys_tr

        ## val forward pass
        with torch.no_grad():
            loss_x_val, loss_dx_val, loss_hys_val, z_t_1_seq_val_pred, x_t_1_seq_val_pred, dx_t_1_seq_val_pred = comprehensive_forward_pass(mdl_hysteresis, x_t_seq_val,
                x_t_1_seq_val, dx_t_seq_val, dx_t_1_seq_val, ddxg_t_seq_val, ddxg_t_1_seq_val, target_seq, dt, x_mean, x_std,
                    dx_mean, dx_std, ddxg_mean, ddxg_std, loss_fn)
    ...
    # save mdl
    save_dict = {}
    save_dict['mdl_hysteresis'] = mdl_hysteresis.state_dict()
...
def open_loop_forecast(x, dx, ddxg, source_seq, mdl_hysteresis, dt, x_mean, x_std,
     dx_mean, dx_std, ddxg_mean, ddxg_std):
    x_t_open = torch.stack([x[:source_seq]], dim=1) # (sequence, batch=1, N_inputs)
    dx_t_open = torch.stack([dx[:source_seq]], dim=1) # (sequence, batch=1, N_inputs)
    ddxg_t_open = ddxg[:source_seq].unsqueeze(1) # (sequence, batch=1, N_inputs=1)
    ddxg_t_1_open = ddxg[source_seq:].unsqueeze(1) # (sequence, batch=1, N_inputs=1)
    
    z_t_1_open_pred, x_t_1_open_pred, dx_t_1_open_pred = seq_pred(mdl_hysteresis, x_t_open, dx_t_open, ddxg_t_open, ddxg_t_1_open, ddxg_t_1_open.shape[0], dt,
     x_mean, x_std,
     dx_mean, dx_std, ddxg_mean, ddxg_std)

    x_t_1_open = x[source_seq:]
    dx_t_1_open = dx[source_seq:]
...
def comprehensive_forward_pass(mdl_hysteresis, x_t_seq, x_t_1_seq, dx_t_seq, dx_t_1_seq, ddxg_t_seq, ddxg_t_1_seq, target_seq, dt, x_mean, x_std,
     dx_mean, dx_std, ddxg_mean, ddxg_std, loss_fn):
    # target sequence prediction 
    z_t_1_seq_pred, x_t_1_seq_pred, dx_t_1_seq_pred = seq_pred(mdl_hysteresis, x_t_seq, dx_t_seq, ddxg_t_seq, ddxg_t_1_seq, target_seq, dt, x_mean, x_std,
     dx_mean, dx_std, ddxg_mean, ddxg_std)
    ...
    z_t = time_delay_hysteresis(x_t_seq, dx_t_seq, mdl_hysteresis) # (batches, states)
    ...
    return loss_x, loss_dx, loss_hys, z_t_1_seq_pred, x_t_1_seq_pred, dx_t_1_seq_pred
...
def seq_pred(mdl_hysteresis, x_t_seq, dx_t_seq, ddxg_t_seq, ddxg_t_1_seq, target_seq, dt, x_mean, x_std, dx_mean, dx_std, ddxg_mean, ddxg_std):
    z_t_1_seq_pred_list = []
    x_t_1_seq_pred_list = []
    dx_t_1_seq_pred_list = []

    for nstep in range(target_seq):
        z_t = time_delay_hysteresis(x_t_seq, dx_t_seq, mdl_hysteresis)
        x_t = x_t_seq[-1]
        dx_t = dx_t_seq[-1]
        ddxg_t = ddxg_t_seq[-1]
        ...
        # predictor
        x_t_1, dx_t_1, ddx_t = EOM_predictor(de_normalize_data(x_t, x_mean, x_std), de_normalize_data(dx_t, dx_mean, dx_std), 
            de_normalize_data(z_t, ddxg_mean, ddxg_std), de_normalize_data(ddxg_t, ddxg_mean, ddxg_std), dt)

        x_t_seq_temp = torch.cat([x_t_seq[1:], x_t_1.unsqueeze(0)], dim=0)
        dx_t_seq_temp = torch.cat([dx_t_seq[1:], dx_t_1.unsqueeze(0)], dim=0)
        z_t_1 = time_delay_hysteresis(x_t_seq_temp, dx_t_seq_temp, mdl_hysteresis)
        x_t_1, dx_t_1 = EOM_corrector(de_normalize_data(x_t, x_mean, x_std), de_normalize_data(dx_t, dx_mean, dx_std), dx_t_1, de_normalize_data(z_t_1, ddxg_mean,
         ddxg_std),
         de_normalize_data(ddxg_t_1, ddxg_mean, ddxg_std), ddx_t, dt) 

        x_t_1, _, _ = normalize_data(x_t_1, x_mean, x_std)
        dx_t_1, _, _ = normalize_data(dx_t_1, dx_mean, dx_std)
        z_t_1, _, _ = normalize_data(z_t_1, ddxg_mean, ddxg_std)
        ...
        z_t_1_seq_pred_list.append(z_t_1) # next step prediction. (batch, states)
...
def EOM_corrector(x_t, dx_t, dx_t_1, z_t_1, ddxg_t_1, ddx_t, dt):
    # corrector
...
def EOM_predictor(x_t, dx_t, z_t, ddxg_t, dt):
    # (batches, states)
    x_t_1 = x_t + dt*dx_t 
    ...
    return x_t_1, dx_t_1, ddx_t

def time_delay_hysteresis(x_t_seq, dx_t_seq, mdl_hysteresis):
    # time-delay transformation for latent hysteresis variable
    # x_t_seq: (sequence, batch, N_inputs)
    ...
    z_t = mdl_hysteresis(torch.cat([x_t_td, dx_t_td], -1)) # (batches, states)
    return z_t 
